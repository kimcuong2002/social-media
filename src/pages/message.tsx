...
import { useGetAllMessage, useGetAllRoom, useGetRoomById, useQueryInfoUser } from '@/features';
...
export const Message = () => {
  ...
  const [getAllMessage, {data: allMessage, loading: loadingMessage}] = useGetAllMessage()
  ...
  const getRooms = async () => {
    return await getAllRoom({variables: { idUser: userData?.getInfoUser.id }});
  }
  ...
  return (
    <Box className="flex">
      ...
      <Box className="w-full">
        <ContactDetail loadingMessage={loadingMessage} userInfo={userInfo} membersOfRoom={membersOfRoom} idRoom={infoRoom?.getRoomById.id} allMessage={allMessage?.getAllMessage}  />
      </Box>
    </Box>
  );
};
